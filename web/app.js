...
async function startRec() {
  ...
  try {
    ...
    recorder.onstop = () => {
      ...
      recorder = null;
      if (blob.size > 1200) askVoice(blob);
      else $("micState").textContent = "Too short — hold longer";
    };
    recorder.start();
    ...
    // getUserMedia needs a secure context; on a bare-IP deploy this is the failure.
    fail("Microphone unavailable. Needs HTTPS or localhost — the text box still works.");
  }
}
...
function stopRec() {
  ...
  $("micHint").textContent = "or type a question below";
}
